...
// // ⚖️ 2. RESOLVE DISPUTE (THEFT REPORT)
...
//       // 🕊️ Maf kar diya. Saman safe hai.
//       item.status = "available";
//        item.adminCourt.verdict = "innocent"; 
    
//     // Ab user se related data saaf karo
...
//         // 🔥 Mandate cancel kar do taaki aage paise na katen
//         if (borrow.mandateDetails?.subscriptionId) {
//            await razorpay.subscriptions.cancel(borrow.mandateDetails.subscriptionId);
//         }
//       }
//       return res.json({ success: true, message: "Case Dismissed! Mandate Cancelled." });

//     } else if (decision === "GUILTY") {
//       // 🔨 Bhaag gaya! Penalty lagao.
//       const penaltyAmount = item.deposit || 10000;
//       let paymentSuccess = false;
//       let rzpErrorMsg = "";

//       // 1. 🤖 TRY AUTOMATIC DEDUCTION (Razorpay Mandate)
...
//                   item: {
//                       name: `Penalty for Damage/Theft: ${item.title}`,
//                       amount: penaltyAmount * 100, // Paise mein
//                       currency: "INR"
//                   }
//               });
//               // Katne ke baad cancel kar do
//               await razorpay.subscriptions.cancel(borrow.mandateDetails.subscriptionId);
//               paymentSuccess = true;
//           } catch (rzpError) {
//               console.error("Razorpay Charge Failed:", rzpError);
//               rzpErrorMsg = "Mandate Failed (Bank account empty).";
//           }
//       } else {
//           rzpErrorMsg = "No active mandate found.";
//       }

//       // 2. 🛡️ PLATFORM GUARANTEE: LENDER KO PAISE DO (Chahe bank se kate ya nahi)
//       item.status = "stolen"; 
//       item.adminCourt.verdict = "guilty"; // 🔥 Ye Frontend ko "PAID" dikhayega
// item.adminCourt.adminNote = adminNote;
//       await item.save();

//       if (borrow) {
//           borrow.totalPrice = (borrow.totalPrice || 0) + penaltyAmount; 
//           borrow.status = "completed"; // Completed = Paisa seedha Lender ke Wallet mein
          
//           // Database mein note likh do taaki tujhe yaad rahe
//           borrow.adminNote = adminNote + (paymentSuccess ? " (Auto Recovered via Bank)" : " (MANUAL RECOVERY NEEDED)");
//           await borrow.save();
//       }
      
//       // 3. 📲 ADMIN KO SAHI MESSAGE BHEJO
//       if (paymentSuccess) {
//           return res.json({ 
//               success: true, 
//               message: `✅ SUCCESS! ₹${penaltyAmount} auto-deducted from Borrower's Bank and added to Lender's Wallet.` 
//           });
//       } else {
//           return res.json({ 
//               success: true, 
//               message: `⚠️ ATTENTION ADMIN: Lender got the money in Wallet, BUT ${rzpErrorMsg} You need to physically recover ₹${penaltyAmount} from Borrower!` 
//           });
//       }
//     }
//   } catch (err) { // 🔥 ASLI FIX: Ye catch block miss ho gaya tha!
...
//     // Security Check: Sirf Owner hi case file kar sakta hai
//     if (item.owner.toString() !== req.user._id.toString()) {
//       return res.status(403).json({ success: false, message: "Only the owner can file a case." });
//     }

//     // Item ko court mein bhej do
//     item.status = "disputed_in_court";
    
//     // DB mein note save kar lo admin ke liye
//     item.adminCourt = {
//       reason: reason,
...
//     res.json({ success: true, message: "🚨 Case filed successfully! Admin will review it now." });
//   } catch (err) {

...
export const register = async (req, res) => {
  try {
    ...
    // 🔥 UPDATED FALLBACK LOGIC START (For Register)
    // Hum check karenge ki WhatsApp message actually gaya ya nahi (True/False)
    let isWaSent = false;
    try {
        isWaSent = await sendWhatsAppOTP(phone, otp);
    } catch (e) {
        console.error("⚠️ WhatsApp API Crash:", e.message);
        isWaSent = false;
    }

    if (isWaSent) {
        // Agar WhatsApp chala gaya
        res.json({ success: true, message: "OTP sent to your WhatsApp number." });
    } else {
        // Agar WhatsApp fail hua (return false) ya crash hua
        console.log("⚠️ WhatsApp Failed. Sending Fallback OTP in response.");
        res.json({ 
            success: true, 
            message: "Network Issue (WhatsApp Failed). Use this OTP.", 
            fallbackOtp: otp // <--- Ye Frontend Alert me dikhega
        });
    }
    // 🔥 FALLBACK LOGIC END
...
// 2. Verify OTP (Same as before - No Changes)
export const verifyRegisterOtp = async (req, res) => {
  try {
    const { phone, otp } = req.body;
    console.log(`🔍 Verification Attempt - Phone: ${phone}, Entered OTP: ${otp}`);
...
// 3. Login (Same as before - No Changes)
export const login = async (req, res) => {
  try {
    const { phone, password } = req.body;
    const user = await User.findOne({ phone });
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    // 🔥 UPDATED FALLBACK LOGIC START (Forgot Password)
    let isWaSent = false;
    try {
        isWaSent = await sendWhatsAppOTP(phone, otp);
    } catch (e) {
...
// 5. Reset Password (Same as before - No Changes)
export const resetPassword = async (req, res) => {
  try {
    const { phone, otp, newPassword } = req.body;
...
export const resendOtp = async (req, res) => {
  try {
    ...
    // 🔥 UPDATED FALLBACK LOGIC START (Resend OTP)
    let isWaSent = false;
    try {
        isWaSent = await sendWhatsAppOTP(phone, otp);
    } catch (e) {

...
export const register = async (req, res) => {
  try {
    ...
    let user = existingUser || new User({ name, phone, password: hashedPassword });
    user.otp = otp;
    user.otpExpiry = otpExpiry;
    user.password = hashedPassword;
    await user.save();
    ...
    try {
      // 🟢 Pehle WhatsApp/SMS try karo
      await sendSMS(phone, otp);
      res.json({ message: "OTP sent successfully", otp: otp, fallbackOtp: otp });
    } catch (err) {
      // 🔴 Fail hone par Fallback OTP
      res.json({ message: "SMS Failed. Use Fallback.", otp: otp, fallbackOtp: otp });
    }
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
// ... Verify OTP and Login remain the same ...
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    try {
      await sendSMS(phone, otp);
      res.json({ message: "OTP sent for password reset", otp: otp, fallbackOtp: otp });
    } catch (err) {
      res.json({ message: "SMS Failed. Use Fallback.", fallbackOtp: otp, otp: otp });
    }
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
export const resendOtp = async (req, res) => {
  try {
    ...
    try {
       await sendSMS(phone, otp);
       res.json({ message: "OTP resent successfully", otp: otp, fallbackOtp: otp });
    } catch(err) {
       res.json({ message: "SMS Failed. Use Fallback.", fallbackOtp: otp, otp: otp });
    }
  } catch (error) {
